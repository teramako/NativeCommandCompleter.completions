...
$vaultIdParam = New-ParamCompleter -LongName vault-id -Description $msg.vault_id -Arguments @{ Name = 'VAULT_ID' }
$vaultPasswordFileParam = New-ParamCompleter -LongName vault-password-file, vault-pass-file -Description $msg.vault_password_file -Arguments @{ Name = 'VAULT_PASSWORD_FILE'; Type = 'File' }
...
Register-NativeCompleter -Name ansible-vault -Description $msg.ansible_vault -Parameters @(
    ...
    New-CommandCompleter -Name encrypt_string -Description $msg.encrypt_string -Parameters @(
        ...
        $vaultPasswordFileParam
        $askVaultPassParam
        New-ParamCompleter -LongName stdin-name -Description $msg.encrypt_string_stdin_name -Arguments @{ Name = 'ENCRYPT_STRING_STDIN_NAME' }
        ...
        New-ParamCompleter -ShortName n -LongName name -Description $msg.encrypt_string_name -Arguments @{ Name = 'ENCRYPT_STRING_NAME' }
        New-ParamCompleter -LongName output -Description $msg.output -Arguments @{ Name = 'OUTPUT_FILE'; Type = 'File' }
        $verboseParam
        $helpParam
    ) -NoFileCompletions
    ...
    New-CommandCompleter -Name rekey -Description $msg.rekey -Parameters @(
        ...
        $vaultPasswordFileParam
        $askVaultPassParam
        New-ParamCompleter -LongName new-vault-id -Description $msg.new_vault_id -Arguments @{ Name = 'NEW_VAULT_ID' }
        New-ParamCompleter -LongName new-vault-password-file -Description $msg.new_vault_password_file -Arguments @{ Name = 'NEW_VAULT_PASSWORD_FILE'; Type = 'File' }
        $verboseParam
        $helpParam
    )
